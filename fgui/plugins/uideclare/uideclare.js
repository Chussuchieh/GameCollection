...
function uideclare(handler) {
    ...
    let declareWriter = new CodeWriter_1.default({ blockFromNewLine: false, usingTabs: true }); //写.d.ts
...
exports.uideclare = uideclare;
function getAllType(classes, memberInfo, className) {
    var _a;
    ...
    if (memberInfo.type.startsWith(classNamePrefix)) {
        ...
        // 其他包里面的组件
        const fileName = (_a = memberInfo.res) === null || _a === void 0 ? void 0 : _a.fileName;
        if (fileName) {
            obj[memberInfo.varName] = `${memberInfo.type} & ` + classNamePrefix+memberInfo.res.owner +"_"+ fileName.substring(0, fileName.lastIndexOf('.xml')) + endNamePrefix;
        }
        else {
...
function exportType(declareWriter, classes, memberInfo, className) {
    const obj = getAllType(classes, memberInfo, className);
    outputType(declareWriter, obj);
}
function outputType(declareWriter, obj) {
    for (let key in obj) {
...
function isPrivateAllKeys(obj, currKey) {
    return Object.keys(obj).every((v) => isPrivateKey(v));
}
function formatVarValue(path) {
    if (!path) {
        // 如果为空字符串，则表示全局发布路径
        path = csharp_1.FairyEditor.App.project.GetSettings('Publish').path;
    }
    const all = csharp_1.FairyEditor.App.project.GetSettings('CustomProperties').elements;
    const match = path.match(/\{(.*)\}/g);
    let result = csharp_1.FairyEditor.App.project.basePath;
    if (!match) {
        return formatUrl(csharp_1.System.IO.Path.Combine(result, path));
    }
...
function formatUrl(path, is2Win = true) {
    if (!path) {
        return '';
    }
    // path = path.replace(/\\\\/g, '/');
